...
import { AlertTriangle, Mail, User, Trash2, CircleCheck, Phone } from "lucide-react"
import PhoneInput from "react-phone-number-input"
import "react-phone-number-input/style.css"
import { shallowEqual, useDispatch, useSelector } from "react-redux"
import { toast } from "react-toastify"
import { getOtp, updateDetail } from "@/app/user-dashboard/_redux/userApi"
import { updateAdminDetail, verifyAdminOtp } from "../_redux/adminApi"
...
export default function AccountDetailsPage() {
  ...
  const handleAccountInfoSubmit = async (values: typeof initialAccountInfo, { setSubmitting }: any) => {
    ...
    await dispatch(updateAdminDetail(data))
  }
  ...
  const handleOtpSubmit = async (values: typeof initialOtp, { setSubmitting, resetForm }: any) => {
    try {
      ...
      const data = {
        otp: values.otpCode,
        email: otpType === "mail" ? pendingPhoneNumber : "",
        phone: otpType === "mail" ? "" : pendingPhoneNumber,
        type: otpType
      }
      await dispatch(verifyAdminOtp(data))
